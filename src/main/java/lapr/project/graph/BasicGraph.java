...
public interface BasicGraph<V,E> {
     ...
     */
    Edge<V,E> getEdge2(V vOrig, V vDest);
     ...
     * @param va two vertices
     * @param vb
     * @return the edge or null if vertices are not in the graph or not connected
     */
    E removeEdge(V va, V vb);

    boolean validVertex(V vOrig);

    V[] allkeyVerts();

    int key(V v);

    Collection<V> adjVertices(V vOrig);

    Edge edge(V vOrig, V vAdj);
}
